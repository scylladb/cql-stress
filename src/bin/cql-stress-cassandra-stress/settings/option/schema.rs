...
impl SchemaOption {
    ...
    fn from_handles(handles: SchemaParamHandles) -> Result<Self> {
        ...
        // Asking for a consistency mode is only meaningful when this binary can read back
        // the mode the driver actually negotiated, and that API lives behind the
        // `strong-consistency` feature. Without it the DDL would still be emitted and the
        // run would then measure whatever it happened to get - the exact plausible-looking,
        // meaningless result set the whole check exists to prevent. Refuse instead.
        #[cfg(not(feature = "strong-consistency"))]
        if consistency.is_some() {
            anyhow::bail!(
                "-schema replication(consistency=...) requires a cql-stress built with the \
                 `strong-consistency` feature, and this binary was not. That feature turns on \
                 the driver's unstable strongly-consistent-tables API, which is what lets a \
                 run verify it is really routing to tablet Raft leaders; it is off by default \
                 so that ordinary runs depend on no unstable driver surface. Rebuild with \
                 `cargo build --features strong-consistency` (the flag also needs \
                 RUSTFLAGS=\"--cfg scylla_unstable\"), or use the dedicated \
                 strong-consistency image."
            );
        }
