...
fn prepare_parser() -> (ParamsParser, RateParamHandles) {
    ...
    let throttle = parser.simple_param(
        "throttle=",
        None,
        "limits the rate of operations per second with a schedule; does not fix the displayed latencies in regards to coordinated-omission problem (i.e. end_time - actual_start_time)",
        false,
    );
    let fixed = parser.simple_param(
        "fixed=",
        None,
        "limits the rate of operations per second with a schedule; displays coordinated-omission-fixed latencies (i.e. end_time - scheduled_start_time)",
        false,
    );
